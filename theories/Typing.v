...
Definition inds ind (l : list inductive_body) :=
  let fix aux n :=
      match n with
      | 0 => []
...
Program
Definition type_of_constructor (Σ : global_context) (c : inductive * nat) (decl : ident * term * nat)
           (H : declared_constructor Σ c decl) :=
  let mind := inductive_mind (fst c) in
  let '(id, trm, args) := decl in
  match lookup_env Σ mind with
  | Some (InductiveDecl _ decl') =>
    substl (inds mind decl'.(ind_bodies)) trm
  | _ => !
  end.
...
Definition max_sort (s1 s2 : sort) : sort :=
  ...
  | sSet, sSet => sSet
  | sType i, sSet => s1
  | sType p, sType q => sType (Pos.max p q)
  | sProp, sType _ => s2
  | sSet, sType _ => s2
  end.
...
(** Constant unfolding *)
| red_delta c decl body (isdecl : declared_constant Σ c decl) :
...
Fixpoint eq_term (t u : term) {struct t} :=
  ...
  | tConstruct i k, tConstruct i' k' => eq_ind i i' && eq_nat k k'
...
Ltac construct :=
  match goal with
    ...
    try (simpl; omega); assert(H':=type_Construct Σ Γ c i _ H); simpl in H';
    clear H; apply H'
  end.
...
Lemma term_env_ind :
  forall P : global_context -> term -> Prop,
    ...
    (forall Σ s, P Σ (tConst s)) ->
    (forall Σ i, P Σ (tInd i)) ->
    (forall Σ (i : inductive) (n : nat), P Σ (tConstruct i n)) ->
